...
void init(ll id, ll st, ll en) {
    ...
    if(t[id*2].F<t[id*2+1].F){
        t[id] = t[id*2];
    }else if(t[id*2].F>t[id*2+1].F){
        t[id] = t[id*2+1];
    }else{
        t[id] = t[id*2];
        t[id].S += t[id*2+1].S;
...
pll ask(ll id, ll st, ll en, ll i, ll j) {
    if (i > en || j < st) return {infLL, 0};
    if (st >= i && en <= j) return t[id];
    ll mid = (st + en) >> 1;
    pll leftMin = ask(id*2, st, mid, i, j);
    pll rightMin = ask(id*2+1, mid+1, en, i, j);
    pll ans;
    if(leftMin.F<rightMin.F) ans = leftMin;
    else if(leftMin.F>rightMin.F)ans = rightMin;
    else{
        ans = leftMin;
        ans.S += rightMin.S;
    }
