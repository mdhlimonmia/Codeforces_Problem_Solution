...
void solve(){
    ...
    while(q--){
        ...
        if(op == 3){
            if(qr[0]<=qr[1]){
                st.update(1, 0, n-1, qr[0], qr[1], qr[2]);
            }else{
                st.update(1, 0, n-1, qr[0], n-1, qr[2]);
                st.update(1, 0, n-1, 0, qr[1], qr[2]);
            }
        }else{
            if(qr[0]<=qr[1]){
                cout<<st.ask(1, 0, n-1, qr[0], qr[1])<<endl;
            }else{
                cout<<min(st.ask(1, 0, n-1, qr[0], n-1) ,st.ask(1, 0, n-1, 0, qr[1]))<<endl;
            }
        }
    }
}
